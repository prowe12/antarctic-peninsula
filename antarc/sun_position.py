# -*- coding: utf-8 -*-
"""
Created on Sun Dec 21 14:20:41 2014

@author: prowe (translated from matlab code see also comments below)
# excuse me! @author: prowe? no no no no no no. @author: Daniel

Actually, this code is translated from a matlab code by author unknown 
and translated from matlab to Python by Daniel Neshyba-Rowe
"""
...
def sun_position(time, lat, lon, alt):
  
  """   
    important:
  
  this code was made by someone who really loves their structure 
  arrays. This can make it difficult to translate. 
  (as Daniel Neshyba-Rowe has found out)
  
  Modified to take as input a python-style datetime instead of a
  structure array, "time"
  ...
  # ´sun_topocentric_zenith_angle_calculationª for a possible modification 
  ...
  #               the code. (changed the ´elevationª field in ´locationª structure
  #               information to ´altitudeª), Vincent Roy
  ...
  # calculate location, a structured variable with "lat," "lon," and 
  # "alt" in it NOTE: location used to be the input to sun_position, but
  # was replaced with it's three components.
  location = empty_class()
  ...
  nutation = nutation_calculation(julian.ephemeris_century)
  ...
  topocentric_sun_position_calculate(earth_heliocentric_position, 
                                     location, observer_local_hour, 
                                     sun_rigth_ascension, 
...
def julian_calculation(time):
  ...
  """if not isstruct(t_input):
    tt = datevec(t_input)
    time.UTC=0#time.UTC=0
    time.year  = tt[0]#time.year = tt[1]
    time.month = tt[1]#time.month = tt[2]
    time.day   = tt[2]#time.day = tt[3]
    time.hour  = tt[3]#time.hour = tt[4]
    time.min   = tt[4]#time.min = tt[5]
    time.sec   = tt[5]#time.sec = tt[6]
  else:
      time = t_input
  # end"""
  
  # Note: Cannot use | here, must use or! PMR, 2016/08/25
  ...
  # Not using time.UTC (it was set to zero anyway)    
  #ut_time = ((time.hour - time.UTC)/24) +\
  #(time.min/(60*24)) + (time.sec/(60*60*24)) # time of day in UT time.
  # All the variables below need to be floats!
  ut_time = time.hour/24. + time.minute/(60.*24.) + time.second/(60.*60.*24.) +\
    time.microsecond/(10**6*60.*60.*24.)       # time of day in UT time. 
  
  
  
  D = time.day + ut_time # Day of month in decimal time, ex. 2sd day of 
                         # month at 12:30:30UT, D=2.521180556
  ...
  julian.day = np.floor(365.25*(Y+4716)) + np.floor(30.6001*(M+1)) + D + B - 1524.5
...
def earth_heliocentric_position_calculation(julian):
  ...
  # WARNING: THe final multiplication needs to be a matrix multiplication!
  L0 = sum(A0 * np.cos(B0 + (C0 * JME)))
  L1 = sum(A1 * np.cos(B1 + (C1 * JME)))
  L2 = sum(A2 * np.cos(B2 + (C2 * JME)))
  L3 = sum(A3 * np.cos(B3 + (C3 * JME)))
  L4 = sum(A4 * np.cos(B4 + (C4 * JME)))
  L5 = A5 * np.cos(B5 + (C5 * JME))

  earth_heliocentric_position =empty_class()
  ...
  R4_terms = np.array([[4., 2.56, 6283.08]])
...
def sun_geocentric_position_calculation(earth_heliocentric_position):
  ...
  sun_geocentric_position = empty_class()
...
def nutation_calculation(julian_ephemeris_century):
  ...
  # 5. Longitude of the asc# ending node of the moon's mean orbit on the
  ...
  tabulated_argument = np.dot(Y_terms,Xi) * np.pi/180
  
  #delta_longitude = ((nutation_terms(:,1) + (nutation_terms(:,2) * JCE))) * np.sin(tabulated_argument)
  #delta_obliquity = ((nutation_terms(:,3) + (nutation_terms(:,4) * JCE))) * np.cos(tabulated_argument)
  delta_longitude = (nutation_terms[:,0] + (nutation_terms[:,1] * JCE)) * \
            np.sin(tabulated_argument)
  delta_obliquity = (nutation_terms[:,2] + (nutation_terms[:,3] * JCE)) * \
            np.cos(tabulated_argument)
  
  # Nutation in longitude
  nutation = empty_class()
...
def true_obliquity_calculation(julian, nutation):
  ...
  p = [2.45, 5.79, 27.87, 7.12, -39.05, -249.67, 
  ...
  p[4]*U**6 + p[5]*U**5 + p[6]*U**4 + p[7]*U**3 + p[8]*U**2 + p[9]*U + p[10]
...
def apparent_sun_longitude_calculation(sun_geocentric_position, 
                                       nutation, aberration_correction):
  # This function compute the sun apparent longitude
  
  apparent_sun_longitude = sun_geocentric_position.longitude + \
    nutation.longitude + aberration_correction
...
def sun_rigth_ascension_calculation(apparent_sun_longitude, true_obliquity, sun_geocentric_position):
  ...
  # after " - " on line 808 was "..." changed to "\" same on line 825.
  argument_numerator = (np.sin(apparent_sun_longitude * np.pi/180) * \
  ...
  sun_rigth_ascension = np.arctan2(argument_numerator, argument_denominator) * \
    180/np.pi
  # Limit the range to [0,360]
  
  #print sun_rigth_ascension
...
def sun_geocentric_declination_calculation(apparent_sun_longitude, true_obliquity, sun_geocentric_position):
  ...
  argument = (np.sin(sun_geocentric_position.latitude * np.pi/180) * \
  np.cos(true_obliquity * np.pi/180)) + \
...
def topocentric_sun_position_calculate(earth_heliocentric_position, location, observer_local_hour, sun_rigth_ascension, sun_geocentric_declination):
  ...
  denominator = np.cos(sun_geocentric_declination * np.pi/180) - (x * np.sin(eq_horizontal_parallax * np.pi/180) * np.cos(observer_local_hour * np.pi/180))
  sun_rigth_ascension_parallax = np.arctan2(nominator, denominator)
  # Conversion to degrees. 
  topocentric_sun_position = empty_class()
...
def sun_topocentric_zenith_angle_calculate(location, topocentric_sun_position, topocentric_local_hour):
  ...
  argument = (np.sin(location.latitude * np.pi/180) * \
  ...
  # refraction_corr = (P/1010) * (283/T) * 1.02 / (60 * np.tan(argument * np.pi/180))
  ...
  denominator = (np.cos(topocentric_local_hour * np.pi/180) * \
  np.sin(location.latitude * np.pi/180)) - \
      (np.tan(topocentric_sun_position.declination * np.pi/180) * np.cos(
      location.latitude * np.pi/180))
...
if __name__ == '__main__':
  import datetime  
  datey = datetime.datetime(2015, 9, 4, 8, 26)
  lat = 47.2414
  lon = 122.4594
  alt = 0.
  sun=sun_position(datey,lat,lon,alt)
"""  
