...
from antarc.pwrf_v_sonde.load_sonde import IgraSonde, MbioSonde_txt
...
# Escudero sonde data (note: Rothera and Marambio handled below)
snd_files = [
...
for snd_file, date_str in zip(snd_files, date_strs):
    cases.append(
        {
            "loc": "Escudero",
            "pwrf_dir": "Feb_2022/pwrf_stations/",
            ...
            "snd_file": snd_file,
            "sndfile_fmt": "esc_sonde_dd_v1_%Y%m%d%H.txt",
            "date_str": date_str,
            "snd_class": DenialSondePR,
        },
    )

# Reminder: file format for PWRF, e.g.
# PWRF_D03_Rothera_rh_20220206_09_hly.nc
# =>                  YYYYMMDD_DD
# where DD and DD are beginning and ending days
# datestr is used to get ERA5 data and for printing result tables
# and should be the same as the sounding date and time
cases += [
    {
        "loc": "Rothera",
        "pwrf_dir": "Feb_2022/pwrf_stations/",
        ...
        "snd_file": "Rothera_igra_20220209_12.txt",
        "sndfile_fmt": "Rothera_igra_%Y%m%d_%H.txt",
        "date_str": "2022/02/09 12 UT",
        "snd_class": IgraSonde,
    },
    {
        "loc": "Marambio",
        ...
        "snd_file": "Marambio_2022020712.txt",
        "date_str": "2022/02/07 12 UT",
        "sndfile_fmt": "Marambio_%Y%m%d%H.txt",
        "snd_class": MbioSonde_txt,
    },
]
...
for idate in range(len(cases)):
    ...
    sondeClass = case["snd_class"]

    # The radiosonde data
    sonde = sondeClass(snd_dir, snd_file, sndfile_fmt)
    ...
    if table_model == "pwrf":
        ...
        # pwrf.trim(sonde.lev)  # Makes no difference
        ...
        # TODO: next line will bomb - is this functionality needed?
        # era.trim(sonde.lev)
    ...
    if table_type == "rh":
        ...
        print(f"{round(maxy,sif)}, ", end="")
        print(f"{round(drh['mean'],sif)}, ", end="")
        print(f"{round(drh['rms'],sif)}, ", end="")
