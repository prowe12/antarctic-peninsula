...
def get_forcing_stats_all(era5, meas, npts):
    """
    Get forcing stats
    """
    if not era5 or (len(meas["lwd_date"]) == 0 and len(meas["swd_date"]) == 0):
        return None, None
    lwd = get_forcing_stats_key(era5, meas, "lwd", npts)
    swd = get_forcing_stats_key(era5, meas, "swd", npts)
    return lwd, swd


def get_forcing_stats_key(era5, meas, key, npts):
    """
    Get broadband stats for lwd or swd
    """
    datestr = key + "_date"
    return get_forcing_stats(
        era5["date"], era5[key], era5[key + "_clr"], meas[datestr], meas[key]
    )
...
def get_forcing_monthly_meas(
    ...
    mon_keys = ["era5", "meas", "bias", "rms", "npts"]
    ...
    for iyr, year in enumerate(years):
        for imo, mon in enumerate(months):
            ...
            # for key in ["lwd", "swd"]:
            #     datestr = key + "_date"
            #     stats = get_forcing_stats(
            #         era5["date"], era5[key], meas_hr[datestr], meas_hr[key]
            #     )

            #     for key2 in mon_keys:
            #             monthly[key][key2][iyr, imo] = stats[key2]

            lwd_stats, swd_stats = get_forcing_stats_all(
                era5, meas_hr, pts_min
            )
