...
for imo, month in enumerate(months):
    for iyr, year in enumerate(years):
        ...
        # Time period (beginning insclusive; end non-inclusive)
        ...
        for fname in files_daterange:
            ...
            with Dataset(lidar_dir + fname) as ncid:
                ...
                ialt = np.where(np.isnan(ncid[altkey]) == False)[0]
                itemp = np.where(np.isnan(ncid[tempkey]) == False)[0]
                alt_km = ncid[altkey][:].data
                ...
                liqtemp[imo][iyr] += list(ncid[tempkey][itemp_liq].data)
                icealt[imo][iyr] += list(alt_km[ialt_ice])
                icetemp[imo][iyr] += list(ncid[tempkey][itemp_ice].data)
                cldalt[imo][iyr] += list(alt_km[ialt_cld])
                cldtemp[imo][iyr] += list(ncid[tempkey][itemp_cld].data)
                ...
                nalt = sum(~np.isnan(ncid["CloudBaseAltitude"]))
                if nalt > 0:
                    ndays_alt[iyr, imo] += 1
                    alt[iyr, imo] += (
                ...
                ntemp = sum(~np.isnan(ncid[tempkey]))
                if ntemp > 0:
...
# Percentages of the month for which each cloud type applies
...
print("Total number of ice, liquid, unknown, and all cloud")
print(totice)
print(totliq)
print(totunkcld)
print(totcld)
...
# Get the fractions of time each cloud mask applies
fclr = 100 * clr / ndays
fice = 100 * ice / ndays
fliq = 100 * liq / ndays
...
ax.text(7.8, 60, "Liquid or Mixed Phase")
ax.text(8.1, 15, "Ice")
...
print(f"Clear fraction: {np.mean(ave_clr[2:5])}")
print(f"Clear fraction: {np.mean(ave_clr[5:8])}")
print(f"Clear fraction: {np.mean(ave_clr[8:11])}")
...
fig, ax = plt.subplots(figsize=[5, 2.6], num=1, clear=True)
...
# # # # # #    Plot probability density for altitude by season   # # # # # #
...
wid = 0.41
height = 0.85
col = ["tab:red", [1, 0.6, 0], "green", [1, 0.8, 0]]

plt.figure()  # figsize=(8.8, 11), num=figno, clear=True)
ax1 = plt.subplot(121)
ax2 = plt.subplot(122)
ax1.set_position([0.09, bot, wid, height])
ax2.set_position([0.56, bot, wid, height])

ax1.hist(
    liq_alt, orientation=flip, density=True, bins=bins, label=season, color=col
)
ax1.set_ylim([0, 10])
ax1.legend()
ax1.set_xlabel("Probability Density")
ax1.set_ylabel("Altitude (km)")
ax1.text(0.05, 9.5, "a) Liquid")

ax2.hist(
    ice_alt, orientation=flip, density=True, bins=bins, label=season, color=col
)
ax2.set_ylim([0, 10])
ax2.legend()
ax2.set_xlabel("Probability Density")
ax2.text(0.05, 9.5, "b) Ice")

if savefigs:
    plt.savefig(savedir + "cloud_base_height_hist.png", format="png")
    plt.savefig(savedir + "cloud_base_height_hist.eps", format="eps")
...
bins = np.array([200, 213, 223, 233, 243, 253, 263, 273, 280]) + 0.15
left = 0.11
wid = 0.87
height = 0.38
col = ["tab:red", [1, 0.6, 0], "green", [1, 0.8, 0]]
...
ax1.hist(liq_temp, density=True, bins=bins, label=season, color=col)
# ax1.set_ylim([0, 10])
ax1.legend()
ax1.set_ylabel("Probability Density")
ax1.text(201, 0.008, f"a) Liquid ({nliq} cases)")

ax2.hist(ice_temp, density=True, bins=bins, label=season, color=col)
# ax2.set_ylim([0, 10])
ax2.legend()
ax2.set_ylabel("Probability Density")
ax2.set_xlabel("Temperature (K)")
ax2.text(201, 0.008, f"b) Ice ({nice} cases)")

if savefigs:
    plt.savefig(savedir + "cloud_base_temperature_hist.png", format="png")
...
fix, axs = plt.subplots(2, 1, figsize=[8, 8], num=7, clear=True)
