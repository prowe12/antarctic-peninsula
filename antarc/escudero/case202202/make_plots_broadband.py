...
from antarc.escudero.case202202.get_swd_flux import get_obs_swd
from antarc.escudero.case202202.get_swd_flux import get_pysolar_swd
from antarc.escudero.case202202.get_swd_flux import get_libradtran
from antarc.escudero.case202202.get_lwd_flux import get_lwd, get_lwd_clear
from antarc.escudero.parameters import swd_params

# from antarc.escudero.case202202.get_swd_flux import get_swd
...
def plot_measured_and_clear():
    ...
    ax1.plot(swd_date_long[:iswl], swd_long[:iswl], "r", label="Measured")
    ax1.plot(swd_date_long[iswl:], swd_long[iswl:], color="red")
    ax1.plot(
        swd_date_long[:iswl], swd_clear_lib_long[:iswl], "k--", label="Clear"
    )
    ax1.plot(swd_date_long[iswl:], swd_clear_lib_long[iswl:], "k--")
    # global down
    ...
    ax1.legend()
    ax1.set_xlim(
        dt.datetime(2022, 2, 5, tzinfo=utc),
        dt.datetime(2022, 2, 11, tzinfo=utc),
    )
    ax1.set_ylim([0, 1000])
...
def plot_meas_clear_forcings_cumulative_shorttime():
    rot = 40
    # datefmt = "%d %H"
    ...
    ax1.plot(swd_date[:isw], swd_clear_lib_interp[:isw], "k--", label="Clear")
    ax1.plot(swd_date[isw:], swd_clear_lib_interp[isw:], "k--")
    ax1.legend()
    ...
    # Add pysolar result for comparison
    # ax1.plot(
    #     swd_date[:isw], swd_pysolar[:isw], "--", color="blue", label="Pysolar"
    # )
    # ax1.legend()
...
def plot_meas_clear_forcings():
    ...
    ax2.plot(lwd_date[:ilw], lwd[:ilw], color="blue", label="Measured")
    ...
    tot_force = swd_force[:isw] + lwd_force_sw_date[:isw]
    ax3.plot(lwd_date[:ilw], lwd_force[:ilw], color="blue", label="LWD")
    ax3.plot(swd_date[:isw], swd_force[:isw], color="red", label="SWD")
    ax3.plot(swd_date[:isw], tot_force, "k", label="Total")
    ax3.plot(lwd_date[:ilw], np.zeros(ilw), "k:")
    ax3.legend()
...
date1 = dt.datetime(2022, 2, 5, tzinfo=utc)
date2 = dt.datetime(2022, 2, 11, tzinfo=utc)
