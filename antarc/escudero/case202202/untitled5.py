...
if save_libradtran_tofile:
    outfile = out_dir + "libradtran.csv"
    libdate, swd_clear_lib = get_libradtran(swd_date, outfile)
# TODO: write the following code:
# Load the libradtran results
# swd_clear_lib = get_libradtran(swd_date)
swd_clear_lib = swd_clear_lib[:, 1]
...
# Pick off the global downwelling and interpolate to swd_date
...
# ax1.plot(swd_date[:isw], swd_pysolar[:isw], "--", color="red", label="Clear")
# ax1.plot(swd_date[isw:], swd_pysolar[isw:], "--", color="red")
...
ax1.plot(swd_date[:isw], swd_clear_lib_interp[:isw], "k--", label="Clear")
# ax1.plot(swd_date[:isw], swd_pysolar[:isw], "--", color="red", label="Pysolar")
ax1.legend()
